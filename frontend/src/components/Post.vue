    ...
    <div class="card">
      <div v-if="isEditing">
        ...
        <div class="card-body">
          ...
          <div class="edit-button">
            <button class="button" @click="saveEdit(post._id)">Save</button>
      ...
      <div v-else>
        ...
        <div class="card-body">
          ...
          <!--Quentin : userInfo is not fully loaded (async) hence only the last post loaded has a valid boolean the post.username == this.userInfo.name || this.userInfo.admin == 1-->
          <div class="edit-button" v-if="true">
...
<script>
  ...
  export default {
    ...
    mounted() {
      //Quentin : ça serait quand même bien pratique s'il y avait "un" endroit avec l'id de l'utilisateur actuel au lieu d'avoir a requéter à CHAQUE post affiché cette information...
      //Et du coup, vu qu'il manque l'info (délai/async) les booléens ne sont pas encore résolvables.
      axios
    ...
    methods: {
      ...
      saveEdit() {
        ...
        axios.post("http://localhost:5000/upload", formData);//And if upload fails ?
        let updatePost = {
          img: this.file.name,
          text: this.text,
        };
        axios.post("http://localhost:5000/updatePost/" + id, updatePost);
        //then we recieved updated info about the post from the back and we update this post and switch back to
        this.isEditing = false;
      },
      deletePost() {
        const id = this.id;
